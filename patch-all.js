...
// 2. Remove Revenue Summary HTML Block
const revenueHtmlRegex = /<!-- Revenue Summary -->[\s\S]*?<\/table>\s*<\/div>/;
content = content.replace(revenueHtmlRegex, '');

// 3. Fix blocks array globally in JS
content = content.replace(/const blocks = \['A', 'B', 'C', 'D'\];/g, "const blocks = ['A1', 'A2', 'A3', 'A4', 'A5', 'A7'];");

// 4. Fix reserved -> partial logic in generateReport
...
// 5. Inject summary stats updates and moveout logic, and REMOVE renderRevenueTable call
const renderTablesRegex = /renderTopBlocksTable\(blockStats\);\s*renderRevenueTable\(\);\s*renderMaintenanceTable\(totalMaintenance, completedMaintenance, inProgressMaintenance, pendingMaintenance\);\s*renderApplicationsTable\(totalApps, approvedApps, pendingApps, rejectedApps\);/;
const renderTablesNew = `renderTopBlocksTable(blockStats);
            renderMaintenanceTable(totalMaintenance, completedMaintenance, inProgressMaintenance, pendingMaintenance);
...
content = content.replace(renderTablesRegex, renderTablesNew);

// 6. Update renderPieChart definition and legend
content = content.replace(
...
// 7. Remove renderRevenueTable function definition
const renderRevenueFuncRegex = /function renderRevenueTable\(\) \{[\s\S]*?\}\s*function renderMaintenanceTable/;
content = content.replace(renderRevenueFuncRegex, "function renderMaintenanceTable");

// 8. Update Quick Report JS Logic
const quickReportRegex = /window\.generateQuickReport = \(type\) => \{[\s\S]*?\};/;
const quickReportNew = `window.generateQuickReport = (type) => {
            const pages = {
                'occupancy': 'warden-rooms.html',
                'maintenance': 'warden-maintenance.html',
                'applications': 'warden-applications.html',
                'moveout': 'warden-moveout.html'
            };
            if (pages[type]) {
                window.location.href = pages[type];
            }
        };`;
content = content.replace(quickReportRegex, quickReportNew);

// 9. Fix top block default
content = content.replace("let topBlock = 'A';", "let topBlock = 'A1';");

fs.writeFileSync('warden-reports.html', content);
console.log('Patched all completely successfully');
