...
if (fs.existsSync('room-apply.html')) {
    ...
    // 1. Replace the entire CSS block for Floor Plan
    const cssStart = "/* Floor Plan Styles */";
    const cssEnd = "margin-left: 8px;\\n        }";
    
    const startIndexCss = content.indexOf(cssStart);
    const endIndexCss = content.indexOf(cssEnd, startIndexCss);
    ...
    if (startIndexCss !== -1 && endIndexCss !== -1) {
        ...
        content = content.substring(0, startIndexCss) + newFloorPlanCSS + content.substring(endIndexCss + cssEnd.length);
    }

    // 2. Replace the HTML block
    const htmlStart = '<div class="floor-plan-container">';
    const htmlEnd = '<input type="hidden" id="selectedPartitionInput" required>';
    
    const startIndexHtml = content.indexOf(htmlStart);
    const endIndexHtml = content.indexOf(htmlEnd, startIndexHtml);
    ...
    if (startIndexHtml !== -1 && endIndexHtml !== -1) {
        ...
        content = content.substring(0, startIndexHtml) + newFloorPlanHTML + content.substring(endIndexHtml + htmlEnd.length);
    }
    
    // 3. Update the JavaScript Logic
    const jsStart = "// Handle Partition Selection";
    const jsEnd = "selectedPartitionInput.value = '';\\n        }";
    
    const startIndexJs = content.indexOf(jsStart);
    const endIndexJs = content.indexOf(jsEnd, startIndexJs);

    if (startIndexJs !== -1 && endIndexJs !== -1) {
        const newJsLogic = `// Handle Partition Selection
        // Re-select blocks because they might have been replaced (though not in this static context, it's good practice)
        ...
        content = content.substring(0, startIndexJs) + newJsLogic + content.substring(endIndexJs + jsEnd.length);
    }

    fs.writeFileSync('room-apply.html', content);
    console.log("Floor plan V2 applied successfully.");
}
