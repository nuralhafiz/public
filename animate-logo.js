...
if (fs.existsSync('index.html')) {
    ...
    // 1. Fix header-overlay starting transform to prevent jumping
    content = content.replace(/\.header-overlay\s*{\s*position:\s*absolute;\s*top:\s*60px;\s*left:\s*50%;\s*transform:\s*translateX\(-50%\);/g, 
        '.header-overlay {\n            position: absolute;\n            top: 120px; /* Compensate for -50% Y */\n            left: 50%;\n            transform: translate(-50%, -50%);');

    // 2. Update the inject-morph logic to animate the logo
    const oldMorphRegex = /const logo = document\.querySelector\('\.header-overlay'\);[\s\S]*?(?=\/\/ We also need to translate)/; // Just in case, let's replace the whole function instead
    ...
    const newFunc = `function triggerMorphTransition(event, targetUrl, targetSectionId) {
            ...
            if (logo) {
                ...
                logo.style.transition = 'all 0.4s cubic-bezier(0.25, 1, 0.5, 1)';
                
                if (isMobile) {
                    logo.style.top = '19vh'; // Center of the 38vh image section
                    logo.style.left = '50%';
                } else {
                    logo.style.top = '50%';
    ...
    console.log("Logo animation logic applied to index.html!");
}
